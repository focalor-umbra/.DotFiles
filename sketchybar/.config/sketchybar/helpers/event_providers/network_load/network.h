...
static inline void network_update(struct network *net)
{
  ...
  if (time_scale < 1e-6 || time_scale > 1e2)
    return;
  double delta_ibytes = (double)(net->data.ifmd_data.ifi_ibytes - ibytes_nm1) / time_scale * 8.0; // Convert bytes to bits
  double delta_obytes = (double)(net->data.ifmd_data.ifi_obytes - obytes_nm1) / time_scale * 8.0; // Convert bytes to bits
